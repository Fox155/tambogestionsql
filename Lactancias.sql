...
SALIR: BEGIN
  ...
  IF EXISTS(SELECT NroLactancia FROM Lactancias WHERE IdVaca = pIdVaca AND (pFechaInicio < FechaInicio OR pFechaInicio < FechaFin)) THEN
    ...
    START TRANSACTION;
        -- Insercion
        SET pIdLactancia = (SELECT COALESCE(MAX(NroLactancia), 0)+1 FROM Lactancias);
...
DELIMITER $$
CREATE PROCEDURE `tsp_finalizar_lactancia`(pIdLactancia Tinyint,pFechaFin date)
/*
...
-- Manejo de error en la transacción
  ...
  IF (pFechaFin IS NULL OR pFechaFin = '') THEN
	...
	END IF;
    -- Control de Parámetros incorrectos
  IF EXISTS(SELECT NroLactancia FROM Lactancias WHERE NroLactancia = pIdLactancia AND FechaInicio > pFechaFin) THEN
  ...
  END IF;
	...
	-- 	SELECT 'Esa Lactancia ya posee una fecha de Fin' Mensaje;
	-- 	LEAVE SALIR;
	-- END IF;
  IF EXISTS(SELECT NroLactancia FROM Producciones WHERE  (NroLactancia = pIdLactancia AND pFechaFin < FechaFin)) THEN
    SELECT 'Esa Lactancia posee producciones que exceden a la fecha de fin' Mensaje;
    LEAVE SALIR;
  END IF;
  START TRANSACTION;
			UPDATE  Lactancias
