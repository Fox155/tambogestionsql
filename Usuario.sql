...
SALIR:BEGIN
    ...
    */
    ...
    IF (pEmail IS NULL OR pEmail = '') THEN
        ...
        LEAVE SALIR;
	END IF;
    IF (pIdTipoUsuario IS NULL OR NOT EXISTS(SELECT IdTipoUsuario FROM TiposUsuarios WHERE IdTipoUsuario = pIdTipoUsuario)) THEN
        SELECT 'El tipo de usuario seleccionado es inexistente.' Mensaje;
        LEAVE SALIR;
	END IF;
	-- IF NOT EXISTS(SELECT IdTipoUsuario FROM Roles WHERE IdTipoUsuario = pIdTipoUsuario AND Estado = 'A') THEN
    --     SELECT 'El tipo de usuario seleccionado está dado de baja.' Mensaje;
    --     LEAVE SALIR;
	-- END IF;

	SELECT IdUsuario INTO pIdUsuarioAud
	FROM Usuarios u INNER JOIN Tambo t USING(IdTambo)
	...
	IF pIdUsuario IS NULL OR pIdUsuario = 0 THEN
        ...
        LEAVE SALIR;
	ELSEIF EXISTS (SELECT IdTambo FROM Tambos t INNER JOIN Usuarios u USING(IdTambo) WHERE t.Estado != 'A' AND u.IdUsuario = pIdUsuario) THEN
		SELECT 'Usted no puede ingresar, su tambo no está habilitada.' Mensaje;
		LEAVE SALIR;
    END IF;
    ...
    START TRANSACTION;
        ...
        INSERT INTO Usuarios VALUES (pIdTambo, pIdTipoUsuario, pUsuario, pEmail, pPassword, pToken, 0, pNow, 'S', 'A');
		-- Audita
		-- INSERT INTO aud_Usuarios
...
DELIMITER $$
CREATE PROCEDURE `tsp_dame_password_hash`(pUsuario varchar(100))
BEGIN
	/*
...
DROP PROCEDURE IF EXISTS `tsp_dame_usuario_por_token`;
DELIMITER $$
CREATE PROCEDURE `tsp_dame_usuario_por_token`(pToken varchar(500))
BEGIN
	/*
    ...
    */
    IF EXISTS (SELECT IdUsuario FROM Usuarios WHERE Token = pToken AND IdTipoUsuario IS NOT NULL) THEN
...
DELIMITER $$
CREATE PROCEDURE `xsp_darbaja_usuario`(pToken char(32), pIdUsuario int, pIP varchar(40), pUserAgent varchar(255), pAplicacion varchar(50))
SALIR: BEGIN
	/*
...
DROP PROCEDURE IF EXISTS `tsp_puede_ejecutar`;
DELIMITER $$
CREATE PROCEDURE `tsp_puede_ejecutar`(pToken varchar(500), OUT pMensaje varchar(255), OUT pIdUsuario bigint)
BEGIN
	/*
    ...
    */
    ...
    INTO		pIdUsuario
    FROM		Usuarios u
    INNER JOIN	Tambo t USING(IdTambo)
	INNER JOIN	TiposUsuarios tu USING(IdTipoUsuario)
    WHERE		u.Token = pToken AND u.Estado = 'A'
	LIMIT		1;
